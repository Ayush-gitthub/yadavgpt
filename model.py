...
eval_interval = 500  # How often to evaluate loss. Set high for efficiency.
eval_iters = 200     # Number of batches to average for a smooth loss estimate.
learning_rate = 3e-4 # Step size for the optimizer.

# Model Architecture Parameters
n_embd = 384         # The "width" of the model; size of token embeddings.
n_head = 6           # Number of parallel self-attention heads.
n_layer = 6          # Number of stacked Transformer blocks (depth of the model).
...
model = LanguageModel()
m = model.to(device)
print(f"{sum(p.numel() for p in m.parameters())/1e6:.2f}M parameters") # Print number of parameters
